...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut sync_target_peer: Option<PeerId> = None;
            ...
            event = swarm.select_next_some() => {
                match event {
                                ...
                                match request {
                                    ...
                                    SyncMessage::MetaDataRequest { file_name } => {
                                        ...
                                        let response = if let Some((_, metadata)) = cached_chunks.get(&file_name) {
                                            ...
                                            match crate::storage::get_file_metadata(&sync_path_abs, &file_name).await {
                                                Ok(msg) => msg,
                                                Err(e) => {
                                                    SyncMessage::Error {
                            ...
                            request_response::Message::Response { response, .. } => {
                                match response {
                                    SyncMessage::FileList { file_names } => {
                                        ...
                                        while sync_session.can_start_more() {
                                            if let Some(file_name) = sync_session.start_next() {
                                                ...
                                                if let Some(target) = sync_target_peer {
                                                    swarm.behaviour_mut().rr.send_request(
                                                        &target,
                                                        SyncMessage::MetaDataRequest { file_name },
                                                    );
                                    ...
                                    SyncMessage::ChunkResponse { chunk_index, data, hash, file_name } => {
                                        let transfer_needed = match transfer_states.get_mut(&file_name) {
                                            Some(state) => state,
                                            None => {
                    ...
                    SwarmEvent::ConnectionClosed { peer_id, .. } => {
                        ...
                        if sync_target_peer == Some(peer_id) {
