...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    loop {
        tokio::select! {
            ...
            // Store the session somewhere - you'll need to add a field to main
            // For now, let's assume you have a current_session: Option<SyncSession>
            current_session = Some(sync_session);
        }
                                        ...
                                        SyncMessage::ChunkResponse { chunk_index, data, hash,file_name } => {
                                            ...
                                            let mut transfer_needed = match transfer_states.get_mut(&file_name) {
                                            Some(state) => state,
                                        None => {
                                                            ...
                                                            Ok(complete) => {
                                                                if complete {
                                                                   ...
                                                                   tokio::time::sleep(tokio::time::Duration::from_millis(200)).await;
                                                                    ...
                                                                    transfer_states.remove(&file_name);

                                                                    if let Some(session) = &mut current_session {

                                                                        session.mark_complete(&file_name);

                                                                        if session.can_start_more() {

                                                                            if let Some(next_file) = session.start_next() {

                                                                                info!("Starting next file {}",next_file);
                                                                                swarm.behaviour_mut().rr.send_request(&peer,SyncMessage::MetaDataRequest { file_name: next_file });
